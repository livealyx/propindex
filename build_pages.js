...
// Extract header and footer from index.html to ensure they match exactly
const indexHtmlPath = path.join(__dirname, 'index.html');
const indexHtmlContent = fs.readFileSync(indexHtmlPath, 'utf8');

// Regex to extract head, nav, and footer
const headMatch = indexHtmlContent.match(/(<head>[\s\S]*?<\/head>)/i);
const navMatch = indexHtmlContent.match(/(<!-- Navigation -->[\s\S]*?<\/nav>)/i);
const footerMatch = indexHtmlContent.match(/(<!-- Footer -->[\s\S]*?<\/footer>)/i);

if (!headMatch || !navMatch || !footerMatch) {
    console.error("Could not find head, nav, or footer in index.html");
    process.exit(1);
}
...
// We need to adjust the title tag dynamically, so let's remove it from headContent
headContent = headContent.replace(/<title>.*?<\/title>/i, '<!-- TITLE_PLACEHOLDER -->');
...
pages.forEach(page => {
    // Only generate if it doesn't exist to avoid overwriting heavily customized pages unless it's a new request
    const filePath = path.join(__dirname, page.file);
    
    const customHead = headContent.replace('<!-- TITLE_PLACEHOLDER -->', `<title>${page.title} | PropIndex</title>`);
